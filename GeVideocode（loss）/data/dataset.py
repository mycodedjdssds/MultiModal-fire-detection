...
class CMHADDataset(Dataset):
    ...
    def build_file_list(self, dir, set):
        ...
        while subject<=1:
            ...
            for m in range(sheet.nrows):
                ...
                dirpath2 = subdir + "/InertialData/inertial_sub" + str(subject) + "_tr" + str(int(sheet.cell_value(m, 0))) + ".csv"
                ...
                if (set == "val") and (sheet.cell_value(m, 0) in valvideo) :
                    ...
                    df = pandas.read_csv(dirpath2, header=None)
                    df = df.astype(float)
                    dflist = df.values.tolist()
                    strii = ','.join(map(str, dflist[int((startframe) / 12 * 64 + 160)]))

                    entry = (int(sheet.cell_value(m, 1) - 1), dirpath, startframe, startframe + 59, subject, strii)
                    ...
                    for n in range(12):
                        ...
                        df = df.astype(int)
                        dflist = df.values.tolist()
                        strii =','.join (map(str , dflist[int((startframe + n) / 12 * 64 + 160)]))

                        entry = (int(sheet.cell_value(m, 1)-1), dirpath, startframe+n, startframe+59+n,subject, strii)
                        completeList.append(entry)
            if set == "test":
                for o in testvideo:
                    startframe = 0
                    dirpath = subdir + "/VideoData/video_sub"+str(subject)+"_tr"+str(o)+".avi"
                    dirpath2 = subdir + "/InertialData/inertial_sub" + str(subject) + "_tr" + str(o)+ ".csv"
                    stri = pandas.read_csv(dirpath2)
                    stri = stri.values.tolist()
                    while startframe <= 920:

                        strii = stri[startframe * 5 + 149]

                        entry = (0, dirpath, startframe, startframe+59,subject,strii)
                        completeList.append(entry)
    ...
    def __getitem__(self, idx):
        ...
        label, filename, startframe, endframe,subject,strii= self.file_list[idx]
        vidframes = load_video(filename, startframe)
        temporalvolume = bbc(vidframes, self.augment)


        sample = {'temporalvolume': temporalvolume, 'label': torch.LongTensor([label]), 'MiddleTime':(startframe+30)/12, 'subject':subject, 'strii':strii}
        return sample
